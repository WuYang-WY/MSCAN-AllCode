...
def save_feature(tensor, scale, name, path=r'E:\experimental\MSCAN\MSCAN_three_level_roi\feature_out/'):
    # print(tensor.shape)
    ...
    if c == 1 or c == 3:
        ...
        dir = os.path.join(path, 'scale_{}_name_{}'.format(scale, name))
        if not os.path.exists(dir):
            os.makedirs(dir)
        for i in range(c):
            feature = tensor[:, i, :, :].unsqueeze(1).cpu() + 0.5
            file_path = os.path.join(dir, '{}.png'.format(i))
            torchvision.utils.save_image(feature, file_path)
